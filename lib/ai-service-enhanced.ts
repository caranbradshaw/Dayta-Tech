...
export async function analyzeUploadedFileEnhanced(
  ...
  userContext?: UserAIContext, // NEW: User context for personalized analysis
  ...
  try {
    ...
    // Get enhanced AI analysis with personalized context
...
`
  ...
  // Get role-specific system prompt with personalization
  const systemPrompt = getRoleSpecificSystemPrompt(context.role, context.industry, context.company, context.region)
  ...
  const message = await anthropic.messages.create({
    ...
    messages: [
      {
        role: "user",
        content: `Create the most comprehensive, professional analysis ever produced for ${context.company} in the ${context.industry} industry from a ${context.role.replace("_", " ")} perspective. This analysis will be presented to senior leadership and must demonstrate exceptional expertise and strategic insight.
...
3. Tailor recommendations specifically for ${context.company}
4. Consider ${context.region} market context and business environment
5. Include quantified business impact
6. Identify competitive advantages and market opportunities
7. Provide executive-level strategic recommendations
8. Include risk assessment and mitigation strategies
${context.role === "data_engineer" ? "9. Include detailed data quality assessment and schema optimization recommendations" : ""}
${context.role === "data_scientist" ? "9. Include statistical significance analysis and machine learning potential assessment" : ""}

This analysis must be the highest quality professional summary ever created for ${context.company} from a ${context.role.replace("_", " ")} perspective.`,
      },
    ],
  })
  ...
  try {
    ...
    return {
      ...result,
      aiProvider: "claude-opus-personalized",
    }
  } catch (error) {
...
Analyze this complete ${context.industry} dataset for ${context.company} with ${processedData.stats.rowCount} rows and ${processedData.stats.columnCount} columns from a ${context.role.replace("_", " ")} perspective. Create a professional analysis that will impress senior leadership.
...
Provide JSON response with executiveSummary, detailedInsights, industrySpecificInsights, roleBasedRecommendations, and ${
  ...
  try {
    ...
    return {
      ...result,
      aiProvider: "gpt-4-turbo-personalized",
    }
  } catch (error) {
    return generateEnhancedIntelligentFallback(processedData, fileName, context)
  }
...
function generateEnhancedIntelligentFallback(
  ...
  const result = {
    ...
    competitiveAnalysis: `Analysis reveals strategic positioning opportunities for ${context.company} within ${context.industry} sector through advanced data capabilities and comprehensive business intelligence infrastructure.`,
    marketTrends: getMarketTrends(context.industry, context.region),
    technicalDetails,
    aiProvider: "enhanced-fallback-personalized",
  }

  return result
}

function getRoleSpecificSystemPrompt(role: string, industry: string, company: string, region: string): string {
...
Your task is to create THE MOST PROFESSIONAL ANALYSIS EVER CREATED for ${company} in the ${industry} industry.
...
function getRoleSpecificOverview(
  ...
  switch (role) {
    case "data_scientist":
      return `Statistical Analysis of ${fileName} reveals a comprehensive ${industry} dataset for ${company} containing ${data.stats.rowCount.toLocaleString()} records across ${data.stats.columnCount} dimensions. This analysis provides advanced statistical insights and machine learning opportunities specifically tailored for ${company}'s ${region} operations, identifying significant correlations, predictive features, and modeling potential. The data quality score of ${data.insights.dataQuality}% supports reliable model development with ${data.stats.numericColumns.length} quantitative variables available for predictive modeling.`

    case "data_engineer":
      return `Technical Analysis of ${fileName} reveals a ${industry} dataset for ${company} with ${data.stats.rowCount.toLocaleString()} records across ${data.stats.columnCount} columns requiring optimization. This analysis identifies data quality issues, schema optimization opportunities, and pipeline improvement recommendations specifically for ${company}'s infrastructure. With ${data.insights.completeness}% data completeness and ${data.anomalies.length} detected anomalies, specific technical interventions can significantly enhance ${company}'s data reliability and performance.`

    case "executive":
      return `Executive Analysis of ${fileName} reveals strategic insights across ${data.stats.rowCount.toLocaleString()} ${industry} data points for ${company}, identifying significant business opportunities and competitive advantages in the ${region} market. This C-level assessment provides quantified business impact analysis, market positioning recommendations, and strategic decision support tailored for ${company}'s leadership team. The comprehensive dataset enables data-driven executive decision-making with ${data.insights.dataQuality}% confidence level.`
